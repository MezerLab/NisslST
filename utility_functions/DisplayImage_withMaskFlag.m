...
%
% This version of the original DisplayImage function includes the option to
% plot only tensor close to identified stained cells.

function DisplayImage_withMaskFlag(im,EigInfo,ConvInfo,para,varargin)
...
if mask_near_cells_flag
    ...
    im_bw = imdilate(im_bw,strel('disk', 23));%round(23*(0.645/0.46))));%round(23*(0.645/0.97)))); % Using the same value as rho makes sense. &&& Change for different datasets
...
figure;   %%% orientations
a1=w1(:,:,2); a2=w1(:,:,1);  
%imshow(im,[]); % I commented this because I don't want to rescale the pixels intensities
imshow(im);
colormap gray;
hold on;
...
P2short=kron([1;1],Centrey)+0.9*scl*kron([1;-1],a2(Cent).*mask_near_cells(Cent));
   ...
   for sI = 1:size(P1,2)
       plot(P1(:,sI),P2(:,sI),'k','linewidth',6);%6*0.645/0.46); %Or for brainmaps: (6*0.65/0.46));% title('tensor orientation');hold off
       h = plot(P1short(:,sI),P2short(:,sI),'linewidth',4); %4*0.645/0.46);% title('tensor orientation');hold off
       h.Color = rgbTmp(sI,:);
   end
